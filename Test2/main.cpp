	...
	_In_ PUNICODE_STRING RegistryPath)
{
	...
	if (!NT_SUCCESS(Status1)) {
		kprintf("[Skengdo] <-> [-] infinityhook: Failed to initialize with status: 0x%lx.\n", Status1);
		return Status1;
	}
	kprintf("[Skengdo] <-> [+] SysCalls: initialized with status: 0x%lx.\n", Status1);
	...
	_Inout_ void** SystemCallFunction)
{
	...
	if (SystemCallIndex == NTGDIDDDDISUBMMITCOMMAND_SYSCALL_INDEX) {
		kprintf("[Skengdo] <-> [+] infinityhook: NtGdiDdDDISubmitCommand 0x1250 SYSCALL %lu: 0x%p [stack: 0x%p].\n", SystemCallIndex, *SystemCallFunction, SystemCallFunction);
		if (*SystemCallFunction != DetourNtGdiDdDDISubmitCommand) {
			...
			*SystemCallFunction = DetourNtGdiDdDDISubmitCommand;
			...
			kprintf("[Skengdo] <-> [+] infinityhook: It is hooked wtf.\n");
		}
	}
}
...
VOID FreeVirtualMemory(PVOID VirtualAddress, SIZE_T Size)
{
	if (MmIsAddressValid(VirtualAddress))
	{
		...
		if (!NT_SUCCESS(Status)) {
			kprintf("[Skengdo] <-> [-] GDI.cpp Warning : Released memory failed.FreeVirtualMemory Internal Function\r\n");
		}
		return;
	}
	kprintf("[Skengdo] <-> [-] GDI.cpp Warning: Released memory does not exist.FreeVirtualMemory Internal Function\r\n");
}
...
BOOL extTextOutW(HDC hdc, INT x, INT y, UINT fuOptions, RECT * lprc, LPWSTR lpString, UINT cwc, INT * lpDx)
{
	...
	if (cwc != 0)
	{
		...
		if (local_lpString != NULL)
		{
			...
			__except (1)
			{
				kprintf("[Skengdo] <-> [-] GDI.cpp Line RtlCopyMemory  Triggers An Error.ExtTextOutW Internal Function\r\n");
				goto $EXIT;
			}
		}
		else
		{
			kprintf("[Skengdo] <-> [-] GDI.cpp Line local_lpString = null  Triggers An Error.ExtTextOutW Internal Function\r\n");
			goto $EXIT;
		}
	}
	...
	if (local_lpDx != NULL)
	{
		...
		{
			kprintf("[Skengdo] <-> [-] GDI.cpp Line local_lpDx = null  Triggers An Error.ExtTextOutW Internal Function\r\n");
		}
	}
	...
	if (NtGdiExtTextOutW != NULL) {
		...
		kprintf("[Skengdo] <-> [-] GDI.cpp Line NtGdiExtTextOutW = NULL Triggers An Error.TextOutW Internal Function\r\n");
	}
$EXIT:
	if (lprc != NULL)
	{
	...
	D3DKMT_SUBMITCOMMAND* data)
{
	...
	//DEBUG info
	kprintf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ \n");
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: Sucessfully hooked \n");
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: Commands %p \n", data->Commands);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: CommandLength %i \n", data->CommandLength);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: SUBMITCOMMANDFLAGS %i %i %i \n", data->Flags.NullRendering, data->Flags.PresentRedirected, data->Flags.Reserved);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: BroadcastContextCount %i \n", data->BroadcastContextCount);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: PrivateDriverDataSize %i \n", data->PrivateDriverDataSize);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: NumPrimaries %i \n", data->NumPrimaries);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: NumHistoryBuffers %i \n", data->NumHistoryBuffers);

	//get process name
	PEPROCESS current_process = IoGetCurrentProcess();
	int pid = (int)PsGetProcessId(current_process);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: Process ID %i \n", pid);
	...
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: Process %s \n", process_name);

	//Get Context
	HDC hdc = NtUserGetDC(NULL);
	if (hdc == NULL) {
		kprintf("[Skengdo] <-> [-] NtGdiDdDDISubmitCommand: The handle is NULL\n");
		return OriginalNtGdiDdDDISubmitCommand(data);
	}
	else {
		kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: The handle have something %p\n", hdc);
	}

	//Draw shit
	HBRUSH brush = NtGdiCreateSolidBrush(RGB(255, 255, 255), NULL);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: Brush %p\n", brush);
	...
	if (drawFillRect) {
		kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: drawFillRect Sucess\n");
	}
	else {
		kprintf("[Skengdo] <-> [-] NtGdiDdDDISubmitCommand: drawFillRect oof \n");
	}
	bool borderSucess = DrawBorderBox(hdc, 0, 200, 100, 200, 3, brush1);
	if (borderSucess) {
		kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: border Sucess \n");
	}
	else {
		kprintf("[Skengdo] <-> [-] NtGdiDdDDISubmitCommand: border oof \n");
	}
	//DEFAULT_CHARSET
	...
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: font %p\n", font);
	HFONT oldfont = NtGdiSelectFont(hdc, font);

	bool textSucess = extTextOutW(hdc, 0, 500, 0, NULL, L"习大大万岁万万岁!", 9, 0);
	if (textSucess) {
		kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: text Sucess\n");
	}
	else {
		kprintf("[Skengdo] <-> [-] NtGdiDdDDISubmitCommand: text oof \n");
	}

	//clean up
	bool deleted = NtGdiDeleteObjectApp(brush);
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: deleted %i\n", deleted);
	...
	kprintf("[Skengdo] <-> [+] NtGdiDdDDISubmitCommand: ReleaseStatus %i \n", releaseStatus);
	kprintf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ \n");
	return OriginalNtGdiDdDDISubmitCommand(data);
}
